//! Native file and folder pickers via `rfd` (ADR-0014).
//!
...
//! native `rfd` UI. A non-empty value is returned as a successful single-path
//! selection; an empty value simulates picker cancellation (`None`). Real picker
//! UI tests without the mock must be `#[ignore]` with reason
//! `requires native picker UI`.

use std::path::{Path, PathBuf};

use rfd::FileDialog;

/// Test-only env var that bypasses the native picker UI (ADR-0014).
...
fn try_mock_paths() -> Option<Option<Vec<PathBuf>>> {
    match std::env::var(MOCK_PICKER_ENV) {
        ...
        Ok(s) => {
            // Support multiple mock paths separated by the OS path list separator.
