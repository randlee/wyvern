...
- `type` must be `"wizard"`
- `page` required: non-empty `id`, `title`, `html`; optional `layout` ∈ `dialog|workspace` (default `dialog`) — see [viewport-sizing.md](viewport-sizing.md); **no validation of tool-specific config keys**
